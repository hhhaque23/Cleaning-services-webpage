/**
 * PastelRibbons — flowing pastel sheets with rainbow refraction strokes.
 *
 * Pure server component: NO framer-motion, NO SVG filters on animated paths.
 * Each ribbon drifts via a CSS keyframe in globals.css (.pr-anim-*) so the
 * motion runs entirely on the GPU compositor — no JS per frame.
 *
 * `prefers-reduced-motion` is honored automatically by the universal rule
 * in globals.css that zeros animation-duration on every element.
 *
 * Mobile (≤768px) is suspended via the same globals.css mobile rule listing
 * each .pr-anim-* class.
 *
 * Layers (back → front):
 ...
 *   4. Large cream sheet filling the lower body
 *   5. Peach accent (mix-blend: screen) drifting through the middle
 *   6. Rose underlay at the bottom
 *   7. Two rainbow refraction strokes along the strongest curves
 */
